...
async def listen_to(stream, object_name):
    print(f"{object_name} stream")
...
def not_async_listen_to(stream, object_name):
    print(f"{object_name} stream")
    for praw_object in stream:
        if praw_object is not None:
            print(praw_object.fullname)
...
async def listen_to_all():
    task1 = asyncio.create_task(listen_to(comment_stream, 'Comment'))
    task2 = asyncio.create_task(listen_to(submission_stream, 'Post'))
    await asyncio.gather(task1, task2)


while True:
    not_async_listen_to(comment_stream,'Comment')
    not_async_listen_to(submission_stream,'Post')
